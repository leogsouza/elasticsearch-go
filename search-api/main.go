...
// Document represents a single document stored in our search engine
...
// DocumentRequest represents a single document in request body
type DocumentRequest struct {
	Title   string `json:"title"`
	Content string `json:"content"`
}

// DocumentResponse represents a response returned after a request
...
// SearchResponse represents a response returned from a search request
...
func searchEndpoint(c *gin.Context) {
	...
	result, err := elasticClient.Search().Index(elasticIndexName).
		Query(esQuery).From(skip).Size(take).Do(c.Request.Context())
